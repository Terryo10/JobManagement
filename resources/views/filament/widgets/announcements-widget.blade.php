<div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm overflow-hidden">
    ...
    <div class="flex items-center justify-between px-6 py-5 border-b border-gray-100 dark:border-gray-700"
         style="background: linear-gradient(135deg, rgba(var(--primary-50), 1) 0%, rgba(var(--primary-100), 0.5) 100%);">
        <div class="flex items-center gap-2.5">
            ...
            <div>
                <h3 class="text-sm font-semibold text-gray-900 dark:text-white leading-tight">Announcements</h3>
        ...
        <div class="flex items-center gap-2">
            <a href="{{ $listUrl }}"
               class="inline-flex items-center gap-1 text-xs text-gray-500 dark:text-gray-400 hover:text-primary-600 dark:hover:text-primary-400 font-medium transition-colors">
                View all
                <x-heroicon-m-arrow-right class="w-3 h-3"/>
            </a>
            <a href="{{ $createUrl }}"
               class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-primary-500 hover:bg-primary-600 text-white text-xs font-semibold shadow-sm transition-colors">
    ...
    @if($announcements->isEmpty())
        <div class="flex flex-col items-center justify-center py-14 text-center px-8">
            <div class="w-12 h-12 rounded-full bg-gray-100 dark:bg-gray-700 flex items-center justify-center mb-3">
                <x-heroicon-o-megaphone class="w-6 h-6 text-gray-400"/>
            </div>
            ...
            <a href="{{ $createUrl }}"
               class="mt-3 inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-primary-500 hover:bg-primary-600 text-white text-xs font-semibold transition-colors">
                <x-heroicon-m-plus class="w-3.5 h-3.5"/>
                Post Announcement
            </a>
        </div>
    @else
        <div class="divide-y divide-gray-100 dark:divide-gray-700/50">
            @foreach($announcements as $announcement)
                <a href="{{ $getViewUrl($announcement) }}"
                   class="flex items-start gap-4 px-6 py-4 hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors group">
                    ...
                    <x-heroicon-m-chevron-right class="flex-shrink-0 w-4 h-4 text-gray-300 dark:text-gray-600 group-hover:text-primary-400 transition-colors mt-1"/>
                </a>
            @endforeach
        </div>
        ...
        <div class="px-6 py-4 bg-gray-50 dark:bg-gray-800/50 border-t border-gray-100 dark:border-gray-700 flex items-center justify-between">
            <span class="text-xs text-gray-400">Showing {{ $announcements->count() }} most recent</span>
            <a href="{{ $listUrl }}"
        ...
        <div class="px-6 py-4 border-t border-gray-100 dark:border-gray-700">
            <a href="{{ $createUrl }}"
               class="flex items-center justify-center gap-2 w-full px-4 py-2.5 rounded-lg bg-primary-500 hover:bg-primary-600 text-white text-sm font-semibold shadow-sm transition-colors">
